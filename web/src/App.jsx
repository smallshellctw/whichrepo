...
const demoProjects = [
  { name: "payments-api", description: "Checkout, payments, and webhook endpoints", languages: ["Go"], dependencies: ["shared-libs"], file_count: 432 },
  { name: "webhook-worker", description: "Asynchronous webhook event processing", languages: ["Go"], dependencies: ["payments-api", "shared-libs"], file_count: 218 },
  { name: "frontend-app", description: "Customer checkout experience", languages: ["TypeScript", "React"], dependencies: ["payments-api"], file_count: 781 },
  { name: "fraud-model", description: "Fraud scoring and payment risk models", languages: ["Python"], dependencies: ["shared-libs"], file_count: 164 },
  { name: "ledger-service", description: "Ledger entries and reconciliation", languages: ["Java"], dependencies: ["payments-api"], file_count: 286 },
  { name: "event-router", description: "Event routing and delivery", languages: ["Rust"], dependencies: ["webhook-worker"], file_count: 129 },
  { name: "billing-api", description: "Subscriptions and invoices", languages: [".NET", "C#"], dependencies: ["payments-api"], file_count: 243 },
  { name: "catalog-service", description: "Product catalog and pricing", languages: ["PHP"], dependencies: ["shared-libs"], file_count: 192 },
  { name: "notification-service", description: "Notification delivery and alerts", languages: ["Ruby"], dependencies: ["webhook-worker"], file_count: 137 },
  { name: "shared-libs", description: "Shared schemas and retry utilities", languages: ["TypeScript"], dependencies: [], file_count: 146 },
];
const demoRoute = { mode: "local_retrieval_only", decision_provider: "local", provider_status: "not_called", primary_project: "payments-api", related_projects: ["webhook-worker", "shared-libs"], needs_clarification: false, change_type: "timeout", candidates: [
  { project: "payments-api", description: "Checkout, payments, and webhook endpoints", local_score: 18.4, semantic_score: 0.92, evidence: ["src/webhooks/checkout_handler.ts", "Handles checkout webhook events and retry behavior.", "RetryPolicy is used by checkout processing."] },
  { project: "webhook-worker", description: "Asynchronous webhook event processing", local_score: 13.7, semantic_score: 0.78, evidence: ["src/processor/retry.go", "Consumes failed webhook jobs."] },
  { project: "shared-libs", description: "Shared schemas and retry utilities", local_score: 8.2, semantic_score: 0.64, evidence: ["src/retry/policy.ts", "Defines shared backoff configuration."] },
] };

function score(candidate, candidates) { if (candidate.semantic_score != null) return Math.round(candidate.semantic_score * 100); const best = Math.max(...candidates.map((item) => item.local_score || 0), 1); return Math.round(((candidate.local_score || 0) / best) * 100); }
function RepoNode({ data }) { const Icon = data.primary ? Database : Folder; return <div className={`repo-node ${data.primary ? "primary" : data.related ? "related" : "muted"}`}><Handle type="target" position={Position.Left} /><span className="repo-icon"><Icon size={20} weight="duotone" /></span><span><strong>{data.label}</strong><small>{data.caption}</small></span><Handle type="source" position={Position.Right} /></div>; }
function SignalNode({ data }) { const icons = { understand: MagnifyingGlass, extract: Code, search: Database, rank: Graph, task: Sparkle }; const Icon = icons[data.kind] || Sparkle; return <div className={`signal-node ${data.kind === "task" ? "task" : ""}`}><Handle type="target" position={Position.Left} /><span className="signal-circle"><Icon size={data.kind === "task" ? 24 : 21} weight="duotone" /></span><strong>{data.label}</strong><Handle type="source" position={Position.Right} /></div>; }
const nodeTypes = { repository: RepoNode, signal: SignalNode };

function SignalMap({ route, projects, task }) {
  const primary = route.primary_project; const related = new Set(route.related_projects || []); const candidates = route.candidates || [];
  const nodes = useMemo(() => { const base = [{ id: "task", type: "signal", position: { x: 5, y: 195 }, data: { label: task.length > 44 ? `${task.slice(0, 41)}…` : task, kind: "task" } }]; const analysis = [["Understand intent","understand"],["Extract signals","extract"],["Search workspace","search"],["Rank evidence","rank"]]; analysis.forEach(([label,kind], index) => base.push({ id: `step-${index}`, type: "signal", position: { x: 250, y: 25 + index * 130 }, data: { label, kind } })); const names = [...new Set([...candidates.map((item) => item.project), ...projects.map((item) => item.name)])].slice(0, 5); names.forEach((name, index) => { const project = projects.find((item) => item.name === name); base.push({ id: `repo-${name}`, type: "repository", position: { x: 515, y: 20 + index * 103 }, data: { label: name, caption: project?.description || candidates.find((item) => item.project === name)?.description || "Repository", primary: name === primary, related: related.has(name) } }); }); return base; }, [route, projects, primary, task]);
  const edges = useMemo(() => { const values = []; for (let index = 0; index < 4; index += 1) values.push({ id: `task-step-${index}`, source: "task", target: `step-${index}`, animated: index < 2, style: { stroke: "#2fd3ff", strokeWidth: 2 } }); nodes.filter((node) => node.id.startsWith("repo-")).forEach((node, index) => { const color = node.data.primary ? "#9dff50" : node.data.related ? "#2fd3ff" : "#3b4a5d"; values.push({ id: `step-repo-${index}`, source: `step-${Math.min(index, 3)}`, target: node.id, animated: node.data.primary || node.data.related, style: { stroke: color, strokeWidth: node.data.primary ? 3 : 1.5 }, markerEnd: { type: MarkerType.ArrowClosed, color } }); }); return values; }, [nodes]);
  return <div className="signal-map" aria-label="Workspace signal map"><ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} fitView minZoom={0.6} maxZoom={1.4} proOptions={{ hideAttribution: true }}><Background color="#223041" gap={28} size={1} /><Controls showInteractive={false} /></ReactFlow></div>;
}
